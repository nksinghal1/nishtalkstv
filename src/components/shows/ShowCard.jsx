...
export default function ShowCard({ show, onClick }) {
  ...
  return (
    <div className="show-card card card-hover" onClick={onClick}>
      <div className="show-card-poster">
        {posterUrl ? (
          <img src={posterUrl} alt={show.title} loading="lazy" />
        ) : (
          <div className="show-card-no-poster">
            <span>📺</span>
          </div>
        )}
        {show.watch_status && (
      ...
      <div className="show-card-body">
        ...
        {genres.length > 0 && (
          <div className="show-card-genres">
            {genres.map(g => (
              <span key={g.id} className="badge badge-muted">{g.name}</span>
            ))}
          </div>
        )}
        ...
        {show.rating && (
          <div className="show-card-rating">
            ...
            <span className="show-card-rating-label">
              {RATING_LABELS[show.rating]}
            </span>
          </div>
        )}
      </div>
